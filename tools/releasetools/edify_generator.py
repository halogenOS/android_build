...
class EdifyGenerator(object):
  ...
  def RunBackup(self, command):
    self.script.append('package_extract_file("install/bin/backuptool.sh", "/tmp/install/bin/backuptool.sh");')
    self.script.append('package_extract_file("install/bin/backuptool.functions", "/tmp/install/bin/backuptool.functions");')
    if not self.info.get("use_set_metadata", False):
      self.script.append('set_perm(0, 0, 0755, "/tmp/install/bin/backuptool.sh");')
      self.script.append('set_perm(0, 0, 0644, "/tmp/install/bin/backuptool.functions");')
    else:
      self.script.append('set_metadata("/tmp/install/bin/backuptool.sh", "uid", 0, "gid", 0, "mode", 0755);')
      self.script.append('set_metadata("/tmp/install/bin/backuptool.functions", "uid", 0, "gid", 0, "mode", 0644);')
